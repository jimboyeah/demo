...
void *CRuntimeClass::CreateObject(char * classname){
    ...
    while(p){
        //cout << p->name << "\t" << p->constructor << endl;
        if( strcmp(classname, p->name) == 0 ) break;
        p = p->pNext;
    }
    if( p==NULL) throw "Not a valid dynamic class specified!";
...
void *Shape::CreateObject(){ return new Shape(); } 
CRuntimeClass Shape::Class = {"Shape", sizeof(Shape), NULL,Shape::CreateObject };
AFX_CLASSINIT Shape::ClassInit(&Shape::Class); 

class Trangle: public Shape {
public:
    Trangle (){ cout << "Trangle" << endl; }
    DECLARE_DYNCREATE
};
